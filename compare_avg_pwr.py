...
# 检查是否有足够的配置列（phy_mode, channel, frequency等）
required_config_cols = ['phy_mode', 'channel', 'frequency']
...
# 排序和筛选差异较大的行（大于0.5 dB）
filtered_df = merged_df[merged_df['abs_diff'] > 0.5]
filtered_df = filtered_df.sort_values('abs_diff', ascending=False)

print('差异大于0.5 dB的行数:', len(filtered_df))

print('\n差异较大的配置和对应值:')
print(filtered_df[required_config_cols + ['avg pwr_260310', 'avg pwr_260309', 'abs_diff']].to_csv(sep='\t', index=False))

# 保存结果到文件
output_file = r'D:/users/gxu/spur_scan/260310/scan_spur_data/xtal_duty_disable/avg_pwr_comparison.txt'
filtered_df[required_config_cols + ['avg pwr_260310', 'avg pwr_260309', 'abs_diff']].sort_values('abs_diff', ascending=False).to_csv(output_file, sep='\t', index=False)
