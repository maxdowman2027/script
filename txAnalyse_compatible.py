...
def spectrum_format(filelist):
    for m in range(0, len(filelist)):
        ...
        # 检查是使用 spectrumMarginDb 还是 spectrumMarginDb_nss1（兼容两种格式）
        if 'spectrumMarginDb' in df.columns:
...
def fltness_check(filelist, logpath):
    f = open(logpath, 'w')
    flatness_check_ok = 1
    for m in range(0, len(filelist)):
        ...
        flatness_num = 0
        is11b = "11b" in filelist[m]
        if not is11b and 'spectralFlatness_margin' in df.columns:
...
def spectrum_check(filelist,logpath):
    ...
    for m in range(0, len(filelist)):
        ...
        if 'spectrumMarginDb' in df.columns:
            spectrumMargin_list = df['spectrumMarginDb']
            spectrumMarginOffsetFreqHz_list = df['spectrumMarginOffsetFreqHz']
        elif 'spectrumMarginDb_nss1' in df.columns:
            # 如果是FPGA格式，这里可能需要调整逻辑，因为FPGA有两个nss列
            f.write('Case ' + filelist[m] + ': FPGA format spectrumMarginDb detected, skipping check\n')
            continue
        else:
            f.write('Case ' + filelist[m] + ': No spectrumMarginDb column detected\n')
            continue
...
def evm_check(filelist,logpath):
    ...
    for m in range(0, len(filelist)):
        ...
        for i in range(-11, 21):
            if 'nss2' in filelist[m]:
                if i in df_evm_nss0_list.index and i in df_evm_nss1_list.index:
...
def crc_check(filelist,logpath):
    ...
    for m in range(0, len(filelist)):
        ...
        if filelist[m] == 'hesu_dcm0.csv' or filelist[m] == 'hesu_stbc.csv' or filelist[m] == 'hesu_nss2.csv':
            ...
            if 'Fail' in mcs8_err_counts and 'Pass' in mcs8_err_counts:
            ...
            if 'Fail' in mcs9_err_counts and 'Pass' in mcs9_err_counts:
...
def tx_plot_and_analyse(logfile,save_filr):
    ...
    for i in range(len(logfile)):
        ...
        # 判断文件类型（FPGA还是芯片测试结果）
        is_fpga_format = 'spectrumMarginDb_nss1' in df.columns and 'spectrumMarginDb_nss2' in df.columns
        ...
        if is_fpga_format:
            ...
            if 'spectrumMarginDb' in df.columns and '--' != df['spectrumMarginDb'].iloc[0]:
                def calculate_worst_spec_margin(x):
                    if x == '--' or pd.isna(x):
                        print(f"Warning: {logfile[i]} : spectrumMarginDb value is '--' for a row, skipping worstSpecMargin calculation.")
                        return np.nan
                    try:
                        ...
                        print(f"Warning: {logfile[i]} : Failed to process spectrumMarginDb value '{x}'. Error: {str(e)}, skipping row.")
                        return np.nan
                df['worstSpecMargin'] = df['spectrumMarginDb'].apply(calculate_worst_spec_margin)
        ...
        if is_fpga_format:
            ...
            if 'worstSpecMargin' in df.columns:
                table_spec_marg = pd.pivot_table(df, index=["tx_power_set(dBm)"],columns=["rate"],values=["worstSpecMargin"])

                spec_marg_columnlist = table_spec_marg.columns
                for column in spec_marg_columnlist:
                    spec_marg_column_lengend_nss1.append(column[1]+'_'+column[0])
                spec_marg_column_lengend_nss1.sort(key=lambda x: int(re.findall(r'\d+', x)[0]))

                for column in spec_marg_column_lengend_nss1:
                    for source_column in table_spec_marg.columns:
                        if column.split('_')[0] == source_column[1].split('_')[0]:
                            reorder_column_spec_marg_nss1.append(source_column)
        ...
        if is_fpga_format:
            ...
            # 芯片格式的标题格式
            if len(logfile[i].split('_')) > 4:
                case_str = logfile[i].split('_')[1] + '_' + logfile[i].split('_')[2] + '_' + logfile[i].split('_')[3] + '_' + logfile[i].split('_')[4] + " " + nss2_flag
            else:
                case_str = logfile[i].split('.')[0] + " " + nss2_flag
        ...
        with open(save_filr+folder_name, 'a') as f:
            ...
            if is_fpga_format:
                ...
                if 'worstSpecMargin' in df.columns:
                    if float(df['worstSpecMargin'].min()) < 0:
                        print(case_str + '      SpecMargin       Check    FAIL\n')
                        print(df[columns_to_print][pd.to_numeric(df['worstSpecMargin'], errors='coerce') < 0].to_string())
                        print("\n")
                    else:
                        print(case_str + "      SpecMargin       Check    PASS")
        ...
        if is_fpga_format:
            if len(logfile[i].split('_')) > 6:
                ...
                title_str = logfile[i].split('_')[1] + '_' + logfile[i].split('_')[2] + '_' + logfile[i].split('_')[3]  + '_' + logfile[i].split('_')[4]  + '_' + 'TxPower' + nss2_flag
        else:
            if len(logfile[i].split('_')) > 4:
                title_str = logfile[i].split('_')[1] + '_' + logfile[i].split('_')[2] + '_' + logfile[i].split('_')[3]  + '_' + logfile[i].split('_')[4]  + '_' + 'TxPower' + nss2_flag
            else:
                title_str = logfile[i].split('.')[0] + '_TxPower' + nss2_flag
        ...
        if '--' != df['amplitude_imbalace'][0]:
            ...
            if is_fpga_format:
                if len(logfile[i].split('_')) > 6:
                    title_str = logfile[i].split('_')[1] + '_' + logfile[i].split('_')[2] + '_' + logfile[i].split('_')[3]  + '_' + logfile[i].split('_')[4]  + '_' + logfile[i].split('_')[5]  + '_' + logfile[i].split('_')[6] + '_' + 'IQ Imbalance'+ nss2_flag
                else:
                    title_str = logfile[i].split('_')[1] + '_' + logfile[i].split('_')[2] + '_' + logfile[i].split('_')[3]  + '_' + logfile[i].split('_')[4]  + '_' + 'IQ Imbalance'+ nss2_flag
            else:
                if len(logfile[i].split('_')) > 4:
                    title_str = logfile[i].split('_')[1] + '_' + logfile[i].split('_')[2] + '_' + logfile[i].split('_')[3]  + '_' + logfile[i].split('_')[4]  + '_' + 'IQ Imbalance'+ nss2_flag
                else:
                    title_str = logfile[i].split('.')[0] + '_IQ Imbalance'+ nss2_flag
        ...
        if is_fpga_format:
            if '--' != df['spectrumMarginDb_nss1'][0]:
                ...
                if len(logfile[i].split('_')) > 4:
                    title_str = logfile[i].split('_')[1] + '_' + logfile[i].split('_')[2] + '_' + logfile[i].split('_')[3] + '_' + logfile[i].split('_')[4]  + '_' + logfile[i].split('_')[5]  + '_' + 'Worst Spectrum Margin nss1'+ nss2_flag
                else:
                    title_str = logfile[i].split('_')[1] + '_' + logfile[i].split('_')[2] + '_' + logfile[i].split('_')[3] + '_' + 'Worst Spectrum Margin nss1'+ nss2_flag
            ...
            if '--' != df['spectrumMarginDb_nss2'][0]:
                ...
                if len(logfile[i].split('_')) > 4:
                    title_str = logfile[i].split('_')[1] + '_' + logfile[i].split('_')[2] + '_' + logfile[i].split('_')[3] + '_' + logfile[i].split('_')[4]  + '_' + logfile[i].split('_')[5]  + '_' + 'Worst Spectrum Margin nss2'+ nss2_flag
                else:
                    title_str = logfile[i].split('_')[1] + '_' + logfile[i].split('_')[2] + '_' + logfile[i].split('_')[3] + '_' + 'Worst Spectrum Margin nss2'+ nss2_flag
            ...
            if 'worstSpecMargin' in df.columns:
                table_spec_marg = table_spec_marg[reorder_column_spec_marg_nss1]
                plt.subplot(6, 1, 3)
                for j in range(len(table_spec_marg.columns)):
                    plt.plot(table_spec_marg.index, table_spec_marg[table_spec_marg.columns[j]].values, 's-',color=custom_colors[int(j)])
                plt.ylim([-10, 30])
                plt.xlim([-12, 20])
                plt.xlabel('ref_power (dBm)')
                ...
                if len(logfile[i].split('_')) > 4:
                    title_str = logfile[i].split('_')[1] + '_' + logfile[i].split('_')[2] + '_' + logfile[i].split('_')[3] + '_' + logfile[i].split('_')[4]  + '_' + 'Worst Spectrum Margin'+ nss2_flag
                else:
                    title_str = logfile[i].split('.')[0] + '_Worst Spectrum Margin'+ nss2_flag
        ...
        if 'spectralFlatness_margin' in df.columns:
            if '--' != df['spectralFlatness_margin'][0]:
                ...
                if is_fpga_format:
                    if len(logfile[i].split('_')) > 6:
                        title_str = logfile[i].split('_')[1] + '_' + logfile[i].split('_')[2] + '_' + logfile[i].split('_')[3]  + '_' + logfile[i].split('_')[4]  + '_' + logfile[i].split('_')[5]  + '_' + logfile[i].split('_')[6] + '_' + 'Worst Flatness Margin'+ nss2_flag
                    else:
                        title_str = logfile[i].split('_')[1] + '_' + logfile[i].split('_')[2] + '_' + logfile[i].split('_')[3]  + '_' + logfile[i].split('_')[4]  + '_' + 'Worst Flatness Margin'+ nss2_flag
                else:
                    if len(logfile[i].split('_')) > 4:
                        title_str = logfile[i].split('_')[1] + '_' + logfile[i].split('_')[2] + '_' + logfile[i].split('_')[3]  + '_' + logfile[i].split('_')[4]  + '_' + 'Worst Flatness Margin'+ nss2_flag
                    else:
                        title_str = logfile[i].split('.')[0] + '_Worst Flatness Margin'+ nss2_flag
        ...
        if is_fpga_format:
            if len(logfile[i].split('_')) > 6:
                title_str = logfile[i].split('_')[1] + '_' + logfile[i].split('_')[2] + '_' + logfile[i].split('_')[3] + '_' + logfile[i].split('_')[4]  + '_' + logfile[i].split('_')[5]  + '_' + logfile[i].split('_')[6] + '_' + 'EVM'+ nss2_flag
            else:
                title_str = logfile[i].split('_')[1] + '_' + logfile[i].split('_')[2] + '_' + logfile[i].split('_')[3] + '_' + logfile[i].split('_')[4]  + '_' + 'EVM'+ nss2_flag
        else:
            if len(logfile[i].split('_')) > 4:
                title_str = logfile[i].split('_')[1] + '_' + logfile[i].split('_')[2] + '_' + logfile[i].split('_')[3] + '_' + logfile[i].split('_')[4]  + '_' + 'EVM'+ nss2_flag
            else:
                title_str = logfile[i].split('.')[0] + '_EVM'+ nss2_flag
...
if __name__ == "__main__":
    ...
    output_dir = r'D:\chip_test\dev\xian_test\Xian-Esp-Test-Scripts\py_script_fpga_tx_wifi7\Log\wifi_tx_rls4\hesu_heersu_260402\result'

    # 文件匹配模式：
    # - 芯片测试结果：使用 '*.csv'
    # - FPGA测试结果：使用 'risc*.csv'
    file_pattern = 'risc*.csv'

    os.chdir(input_dir)
    my_files = sorted(glob.glob(file_pattern), key=os.path.getmtime)

    # 如果没有匹配到文件，尝试其他常见模式
    if not my_files:
        print(f"在 {input_dir} 中没有匹配到 {file_pattern} 文件")
        # 尝试FPGA格式
        my_files = sorted(glob.glob('risc*.csv'), key=os.path.getmtime)
        if my_files:
            print(f"但找到了 {len(my_files)} 个FPGA格式的文件")
        else:
            # 尝试所有CSV文件
            my_files = sorted(glob.glob('*.csv'), key=os.path.getmtime)
            if my_files:
                print(f"但找到了 {len(my_files)} 个CSV文件")
            else:
                print("没有找到任何CSV文件")
                sys.exit(1)
    ...
    # 执行分析
    tx_plot_and_analyse(my_files, output_dir + '/')
