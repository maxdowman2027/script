...
def read_data():
    ...
    try:
        ...
        # 提取所需列的数据（使用实际的列名）
        sample_i_ch0 = df['sample_i_ch0'].values
        sample_q_ch0 = df['sample_q_ch0'].values
        dac_i_ch1 = df['dac_i_ch1[11:0]'].values
        dac_q_ch1 = df['dac_q_ch1[11:0]'].values

        return sample_i_ch0, sample_q_ch0, dac_i_ch1, dac_q_ch1
...
def main():
    ...
    # 读取数据
    sample_i_ch0, sample_q_ch0, dac_i_ch1, dac_q_ch1 = read_data()
    if sample_i_ch0 is None:
        return False

    # 归一化数据
    dac_i_ch0_norm = normalize_data(sample_i_ch0)
    dac_q_ch0_norm = normalize_data(sample_q_ch0)
    dac_i_ch1_norm = normalize_data(dac_i_ch1)
    dac_q_ch1_norm = normalize_data(dac_q_ch1)

    # 创建PDF文件
    output_pdf = r"D:\test_data\wifi7\260327_hesu_nss2\spectrum_plot.pdf"
    with PdfPages(output_pdf) as pdf:
        ...
        # 计算统计数据
        stats_data = [
            ["Ch0 - I", np.mean(sample_i_ch0), np.std(sample_i_ch0), np.min(sample_i_ch0), np.max(sample_i_ch0)],
            ["Ch0 - Q", np.mean(sample_q_ch0), np.std(sample_q_ch0), np.min(sample_q_ch0), np.max(sample_q_ch0)],
