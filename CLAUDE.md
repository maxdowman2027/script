...
| DPD / LUT | **`dpd/xian_static_dpd_main1.py`**（静态/记忆 DPD 训练）；**`dpd/lut_phase0_fit.py`**（读回表后处理：默认 **`smooth`**＝自动坏点+混合 MA+amp 钳位；备选 `repair`/`ma`/`poly`/`iqpoly`）；C 参考 `dpd/lut_phase0_fit.c`；见 **`skill/lut_phase0_fit_Skill.md`** |
...
- **`lut_phase0_fit`**（DPD LUT 读回表后处理）:
  - **`lut_phase0_fit.skill`** — 默认 `smooth`、多 LUT / master、指向完整 MD
  - **`lut_phase0_fit_Skill.md`** — 自动坏点、`smooth` 混合 MA、`repair`/`ma`/`poly`/`iqpoly`、slave 相位约束、HW 写回、变更说明
...
# DPD LUT 读回表后处理（默认 smooth=坏点+混合 MA+amp 钳位；勿默认全表 --method ma）
python dpd/lut_phase0_fit.py "D:\path\to\original_coefficients" -o "D:\path\to\fit_coefficients" --master-lut 0 --scope all
python dpd/lut_phase0_fit.py DIR -o OUT --method smooth --mix-amp 0.5 --mix-ph 0.8 --max-amp-dev 0.10
...
/skill lut_phase0_fit  # DPD lut_data_map 后处理（默认 smooth：坏点+混合 MA）
