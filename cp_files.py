...
def copy_csv_from_matched_folders_recursively(
    ...
    # 自动创建目标路径（如果不存在）
    os.makedirs(target_copy_path, exist_ok=True)
    print(f"目标路径已确认/创建：{target_copy_path}")

    # 统计变量
    total_matched_folders = 0  # 符合条件的文件夹数
    total_scanned_subfolders = 0  # 内层递归扫描的子文件夹数
    total_matched_csv = 0      # 符合条件的CSV文件数
    total_copied = 0           # 成功复制的CSV数
    total_skipped = 0          # 跳过/复制失败的CSV数
    ...
    for dir_path, _, _ in os.walk(root_search_path):
        ...
        for sub_dir_path, _, file_names in os.walk(dir_path):
            ...
            for file_name in file_names:
                ...
                # 处理重名文件（避免覆盖）
                target_csv = os.path.join(target_copy_path, file_name)
                if os.path.exists(target_csv) and not overwrite:
                    file_base, file_ext = os.path.splitext(file_name)
                    suffix = 1
                    while True:
                        new_file_name = f"{file_base}_{suffix}{file_ext}"
                        target_csv = os.path.join(target_copy_path, new_file_name)
    ...
    print(f"✅ 成功复制到目标路径：{total_copied} 个")
    print(f"❌ 跳过/复制失败：{total_skipped} 个")
    print(f"🎯 目标路径：{target_copy_path}")

# ===================== 配置区（请根据你的需求修改）=====================
if __name__ == "__main__":
    # 1. 要递归搜索的根路径（替换为你的实际路径）
    ROOT_SEARCH_PATH = r"D:\chip_test\dev\chip_rx\eagletest\rftest_data\5G\phymd160\160m\ldpc\he"  # Windows示例
    ...
    CSV_PATTERN = "Rx_mcs*_2026030*_*.csv"  # 通配符示例
    # CSV_PATTERN = r"^order_\d{8}\.csv$"  # 正则示例（如order_20260209.csv）
    
    # 4. 复制到的目标路径（替换为你的实际路径）
    TARGET_COPY_PATH = r"D:\users\gxu\spur_scan\260310\5G\160m\he\notch_enable1\he\ldpc"  # Windows示例
    ...
    # 执行核心函数
    copy_csv_from_matched_folders_recursively(
        root_search_path=ROOT_SEARCH_PATH,
        folder_pattern=FOLDER_PATTERN,
        csv_pattern=CSV_PATTERN,
        target_copy_path=TARGET_COPY_PATH,
