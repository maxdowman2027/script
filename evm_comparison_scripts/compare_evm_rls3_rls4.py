...
def main():
    # 文件路径 - 更新为包含rls3和rls4版本的文件
    rls3_file = r"D:\chip_test\dev\chip_tx\eagletest\py_script_fpga_tx\Log\wifi_tx\vht_ht_hesu_rls3\merged_tx_result.xlsx"
    ...
    # 输出目录
    output_dir = r"D:\users\gxu\scripts\evm_comparison_scripts\rls3_rls4_evm_comparison"
    os.makedirs(output_dir, exist_ok=True)

    print("=== Start analyzing RLS3.0 version data ===")
    try:
        rls3_xls = pd.ExcelFile(rls3_file)
        print(f"RLS3.0 version file contains {len(rls3_xls.sheet_names)} Sheets: {rls3_xls.sheet_names}")
    except Exception as e:
        print(f"Failed to read RLS3.0 version file: {e}")
        return

    print("\n=== Start analyzing RLS4.0 version data ===")
    try:
    ...
    # 读取所有Sheet数据
    rls3_data = {}
    for sheet in rls3_xls.sheet_names:
        try:
            df = pd.read_excel(rls3_xls, sheet_name=sheet)
            rls3_data[sheet] = df
            print(f"\nRLS3.0 {sheet} Sheet info:")
            ...
            print(f"Failed to read RLS3.0 {sheet} Sheet: {e}")
    ...
    # Comparison analysis
    comparison_result = []
    for rls3_sheet, rls3_df in rls3_data.items():
        # Find matching RLS4.0 version Sheet
        matched_sheet = find_matching_sheet(rls3_sheet, rls4_data.keys())
        if matched_sheet and matched_sheet in rls4_data:
            print(f"\n=== Comparing {rls3_sheet} (RLS3.0) and {matched_sheet} (RLS4.0) ===")
            compare_dataframes(rls3_df, rls4_data[matched_sheet], rls3_sheet, matched_sheet, comparison_result, output_dir)
        else:
            print(f"\nWarning: No matching RLS4.0 version Sheet found for RLS3.0 {rls3_sheet}")
...
def compare_dataframes(rls3_df, rls4_df, rls3_sheet, rls4_sheet, comparison_result, output_dir):
    # Ensure key columns exist
    required_cols = ['wifi_format', 'rate', 'tx_power_set(dBm)']
    missing_cols = []
    for col in required_cols:
        if col not in rls3_df.columns or col not in rls4_df.columns:
    ...
    # 确定使用哪个EVM列
    rls3_evm_col = 'evm'
    if 'evm' not in rls3_df.columns:
        if 'evm_nss0' in rls3_df.columns:
            rls3_evm_col = 'evm_nss0'
        elif 'evm_nss1' in rls3_df.columns:
            rls3_evm_col = 'evm_nss1'
        else:
            print("Warning: No EVM column found in RLS3.0 data")
            return
    ...
    # Merge data
    merged_df = pd.merge(
        rls3_df[required_cols + [rls3_evm_col]],
        rls4_df[required_cols + ['evm']],
        on=['wifi_format', 'rate', 'tx_power_set(dBm)'],
        how='inner',
        suffixes=('_rls3', '_rls4')
    )

    print(f"Found {len(merged_df)} matching records")

    # 计算EVM差异
    merged_df['evm_diff'] = merged_df['evm_rls4'] - merged_df[f'{rls3_evm_col}_rls3']
    merged_df['abs_diff'] = abs(merged_df['evm_diff'])

    # 保存详细的对比结果，并为EVM值添加填充色
    output_file = os.path.join(output_dir, f'{rls3_sheet}_vs_{rls4_sheet}_detailed.xlsx')
    ...
    # 查找EVM相关列的索引
    rls3_evm_col_idx = None
    rls4_evm_col_idx = None
    evm_diff_col_idx = None
    for idx, cell in enumerate(ws[1]):
        if cell.value == f'{rls3_evm_col}_rls3':
            rls3_evm_col_idx = idx + 1
        elif cell.value == 'evm_rls4':
    ...
    def get_diff_fill(diff_value):
        if diff_value <= -2:
            return PatternFill(start_color='00FF00', end_color='00FF00', fill_type='solid')  # 绿色（RLS4.0 EVM明显更好）
        elif diff_value <= -1:
            return PatternFill(start_color='90EE90', end_color='90EE90', fill_type='solid')  # 浅绿色（RLS4.0 EVM更好）
        elif diff_value <= 1:
            return PatternFill(start_color='FFFF00', end_color='FFFF00', fill_type='solid')  # 黄色（无显著差异）
        elif diff_value <= 2:
            return PatternFill(start_color='FFA500', end_color='FFA500', fill_type='solid')  # 橙色（RLS4.0 EVM较差）
        else:
            return PatternFill(start_color='FF0000', end_color='FF0000', fill_type='solid')  # 红色（RLS4.0 EVM明显较差）

    # 为RLS3.0和RLS4.0的EVM值添加填充色
    if rls3_evm_col_idx:
        for row in range(2, ws.max_row + 1):
            cell = ws.cell(row=row, column=rls3_evm_col_idx)
    ...
    stats = merged_df.groupby(['wifi_format', 'rate']).agg(
        ...
        rls3_mean_evm=(f'{rls3_evm_col}_rls3', 'mean'),
        rls3_median_evm=(f'{rls3_evm_col}_rls3', 'median'),
        rls3_std_evm=(f'{rls3_evm_col}_rls3', 'std'),
        rls4_mean_evm=('evm_rls4', 'mean'),
        rls4_median_evm=('evm_rls4', 'median'),
        rls4_std_evm=('evm_rls4', 'std')
    ).reset_index()

    stats.to_excel(os.path.join(output_dir, f'{rls3_sheet}_vs_{rls4_sheet}_summary.xlsx'), index=False)

    # 可视化
    plot_comparison(stats, merged_df, rls3_sheet, rls4_sheet, output_dir, rls3_evm_col)

    # 更新比较结果列表
    comparison_result.append({
        'rls3_sheet': rls3_sheet,
        'rls4_sheet': rls4_sheet,
        'matched_count': len(merged_df),
        'total_rls3_records': len(rls3_df),
        ...
        'rls3_evm_col': rls3_evm_col
    })


def plot_comparison(stats, merged_df, rls3_sheet, rls4_sheet, output_dir, rls3_evm_col):
    sheet_dir = os.path.join(output_dir, f'{rls3_sheet}_vs_{rls4_sheet}')
    ...
    plt.title(f'{rls3_sheet} vs {rls4_sheet} EVM Difference Distribution')
    ...
    plt.title(f'{rls3_sheet} vs {rls4_sheet} Average EVM Difference by Format and Rate')
    plt.tight_layout()
    plt.savefig(os.path.join(sheet_dir, 'evm_diff_heatmap.png'), dpi=150)
    plt.close()

    # 3. RLS3.0 vs RLS4.0 EVM comparison scatter plot
    plt.figure(figsize=(12, 10))
    plt.scatter(merged_df[f'{rls3_evm_col}_rls3'], merged_df['evm_rls4'], alpha=0.6, s=20)
    plt.plot([merged_df[f'{rls3_evm_col}_rls3'].min(), merged_df[f'{rls3_evm_col}_rls3'].max()],
             [merged_df[f'{rls3_evm_col}_rls3'].min(), merged_df[f'{rls3_evm_col}_rls3'].max()], 'r--', label='Ideal Case')
    plt.xlabel(f'RLS3.0 EVM ({rls3_evm_col}) (dB)')
    plt.ylabel('RLS4.0 EVM (dB)')
    plt.title(f'{rls3_sheet} vs {rls4_sheet} RLS3.0 vs RLS4.0 EVM Comparison')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    ...
    plt.title(f'{rls3_sheet} vs {rls4_sheet} EVM Difference by Wifi Format')
    ...
    plt.axhline(y=0, color='g', linestyle='-', label='No Difference')
    plt.title(f'{rls3_sheet} vs {rls4_sheet} EVM Difference by Rate')
    plt.xlabel('Rate')
    plt.ylabel('EVM Difference (dB)')
    plt.xticks(rotation=45)
    plt.legend()
...
def generate_html_report(comparison_result, output_dir, output_file):
    ...
    <html>
    <head>
        <title>RLS3.0 vs RLS4.0 Version EVM Comparison Analysis</title>
        <style>
            body {
    ...
    <body>
        <div class="container">
            <h1>RLS3.0 与 RLS4.0 版本 EVM 对比分析</h1>
    ...
    for result in comparison_result:
        ...
        total_rls3_records += result['total_rls3_records']
    ...
    html_content += '''
                <div class="stat-card">
                    ...
                    <p>RLS3.0总记录数: %d<br>RLS4.0总记录数: %d</p>
                </div>
                <div class="stat-card">
                    <h4>平均EVM差值</h4>
    ...
    ''' % (
        total_matched, total_rls3_records, total_rls4_records,
    ...
    for result in comparison_result:
        ...
        ''' % (
            ...
            result['rls3_evm_col'],
            result['rls3_sheet'], result['rls4_sheet'],
            result['rls3_sheet'], result['rls4_sheet'],
            result['rls3_sheet'], result['rls4_sheet']
        )
    ...
    html_content += '''
            ...
            <div class="section">
                <h2>3. 主要发现</h2>
                <ul>
                    <li><strong>匹配记录百分比:</strong> RLS3.0记录中有%.2f%%在RLS4.0版本中找到了匹配项</li>
            ...
            <div class="section">
                <h2>5. 文件描述</h2>
                <ul>
                    <li><strong>分析脚本:</strong> compare_evm_rls3_rls4.py</li>
                    <li><strong>RLS3.0版本文件:</strong> D:/chip_test/dev/chip_tx/eagletest/py_script_fpga_tx/Log/wifi_tx/vht_ht_hesu_rls3/merged_tx_result.xlsx</li>
    ...
    ''' % (
        (total_matched / total_rls3_records) * 100,
        "RLS4.0版本整体EVM更好" if avg_abs_diff < -0.5 else "RLS4.0版本整体EVM更差" if avg_abs_diff > 0.5 else "RLS3.0和RLS4.0版本之间的EVM差异不显著",
